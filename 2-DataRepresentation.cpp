...
void DecToBin(int data2, int byt) // Convert Decimal to Binary as Bytes
{
    int count, chk, bits;
...
void Boolean() // Test Data in boolean Mode
{
    ...
    if (data)
        ...
        printf("FALSE\n");                   // Show if false
    printf("Data in Decimal = %uD\n", data); // Show in Decimalformat
    printf("Data in Hexamal = %xH\n", data); // Show in Haxamal format
    printf("Character = %c\n", data);        // Show in Characterformat
    printf("Data in Binary = ");
    DecToBin(data, sizeof(data));              // Show in Binary format
    printf("Size = %d Bytes\n", sizeof(data)); // Show size of unsigned short integer
    getch();                                   // Wait for press any key
...
{
    float data; // Declar data variable for keep Single Precision Float type
    ...
    printf("Data in Binary = ");
    DecToBin(data, sizeof(data));              // Show in Binary format
    printf("Size = %d Bytes\n", sizeof(data)); // Show sizeof unsigned short integer
    getch();                                   // Wait for press any key
}
int main()
{
    ...
    while (ch != '0')
    {
        printf("\n[1]:BinaryToHex [2]:Boolean [3]:UnsignedInt [4]:Character\n");
